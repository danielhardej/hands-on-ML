...
def convolution_layer(input, filter, kernel_size, strides, padding, activation, name):
    conv_layer = tf.layers.conv2d(input, filter=filter, kernel_size=kernel_size,
                             strides=strides, padding=padding,
                             activation=activation, name=name)
    return conv_layer


conv1 = convolution_layer(X_reshaped, filters=conv1_fmaps, kernel_size=conv1_ksize,
                         strides=conv1_stride, padding=conv1_pad,
                         activation=tf.nn.relu, name="conv1")

conv2 = convolution_layer(conv1, filters=conv2_fmaps, kernel_size=conv2_ksize,
                         strides=conv2_stride, padding=conv2_pad,
                         activation=tf.nn.relu, name="conv2")

# print("Convolutional layer 2 generated with: {%i} feature maps, {%i} kernel size, {%i} stride size, {%s} padding".format(conv2_fmaps, conv2_ksize, conv2_stride, conv2_pad) )
